<template>
    ...
    >
      <template v-for="(v, k) in Tabs" :key="k">
        <Tab
          :ModelValue="v"
          :Active="k"
          :class="State.Active == k ? 'blue' : ''"
          @change_active="change_active(k)"
        >
        </Tab>
      </template>
    </van-row>
  ...
  <van-row v-if="State.Active == 0">
    ...
    <van-row class="gray fixed_bottom">
      <van-col
        span="24"
        @click="submit"
        :class="MinReadTime >= Time ? 'disable height50' : 'undisable height50'"
      >
        <i
          :class="`iconfont icon-${Reslut.PaperID ? 'shijuan' : 'bianji'}`"
        ></i>
  ...
  <van-row v-if="State.Active == 2">
    ...
    <template v-if="State.Logs.length > 0">
      <van-list
        ...
        class="block"
      >
        <template v-for="(v, k) in State.Logs" :key="k">
          <LogCard
            :ModelValue="v"
            :Name="yStore.UserMap[v.CUID].Name"
            :Avatar="yStore.UserMap[v.CUID].Avatar"
            :Sex="yStore.UserMap[v.CUID].Sex"
          ></LogCard>
        </template>
      </van-list>
    </template>
  ...
  <!-- 更多 -->
  <van-row v-if="State.Active == 4" class="padding30_top">
    <template v-if="State.Logs.length > 0">
      <van-list
        v-model:loading="Loading"
        :finished="Finished"
...
import { TrainLogApi, TrainsApi, EntityRankLog } from "@yakj/sdk/sdk/sdk";
import { EnumTrainContentType } from "@yakj/sdk/sdk/enum";
import { store, timeout } from "@ctsy/common";
import { Toast, ImagePreview, DialogMessage, Dialog } from "vant";
...
import { ULogs, TrainID, UID, UnitID, PostID, Tabs } from "../../api/Train";
...
const { yStore, Reslut, ShowTab, Time, LearnTime, to_more } = useHead();
const { ShowSign, success, cancel } = useSign();
const { change_active } = useActive();
...
async function onLoadLogs() {
  ...
  switch (State.Active) {
    case 2:
      P.value++;
      let rs = await wait(
        TrainLogApi.logByTrainIDPNUID(P.value, 10, 0, "", "", TrainID)
      );
      t = rs.T;
      State.Logs = rs.L;
