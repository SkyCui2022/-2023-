<template>
  <van-row gutter="6" class="marginbottom15">
  ...
  >
    ...
    <van-row class="fixed_bottom33">
      <van-col span="12">
        <van-button type="danger" size="large" @click="cancal">清空</van-button>
      </van-col>
      <van-col span="12">
...
function cancal() {
  ...
  Props.ModelValue[Key.value].Name = "检查人";
  Props.ModelValue[Key.value].Value = 0;
  RadioData.value = 0;
}

function confrim() {
  ShowPopup.value = !ShowPopup.value;
  emits("select", [Key.value, RadioData.value]);
}
</script>
